...
class Restaurant extends ModeleDeDonnees {
    ...
    function getRestaurant($idRestaurant) : array {
        $reqRest = 'SELECT nom, adresse, cp, ville, telephone, description FROM Restaurants WHERE idRestaurant=:idRestaurant;';
        $prep = $this->executerRequete($reqRest, [':idRestaurant' => $idRestaurant]);
        if($prep->rowCount() === 0) {
